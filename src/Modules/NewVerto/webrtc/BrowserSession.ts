...
import { ICacheDevices, IAudioSettings, IVideoSettings, BroadcastParams, SubscribeParams } from './utils/interfaces'
import { registerOnce, trigger } from './services/Handler'
import { SwEvent, SESSION_ID } from './utils/constants'
import { State, DeviceType } from './constants'
import { getDevices, scanResolutions, removeUnsupportedConstraints, checkDeviceIdConstraints, destructSubscribeResponse, getUserMedia, assureDeviceId } from './helpers'
import { findElementByType } from './utils/helpers'
import { Unsubscribe, Subscribe, Broadcast } from './messages/Verto'
import { localStorage } from './utils/storage/'
import { stopStream } from './utils/webrtc'
...
export default abstract class BrowserSession extends BaseSession {
  ...
  set iceServers(servers: RTCIceServer[] | boolean) {
    ...
    if (typeof servers === 'boolean') {
      this._iceServers = servers ? [STUN_SERVER, TURN_SERVER] : []
    } else {
